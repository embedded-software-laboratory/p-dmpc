...
%     plot_shapes(scenario.dynamic_obstacle_reachableSets); % visualize other vehicles' reachable sets
    ...
    while true
        ...
        if (cur_node_id == -1)
            ...
            info.trims_Hp = del_first_rpt_last(info_prev.trims_Hp); % planned trims in the future Hp time steps
...
%             plot_shapes(info.shapes); % visualize the ego vehicle's predicted occupation areas
        ...
        if info.tree.k(cur_node_id) == scenario.Hp
            ...
            info.trims_Hp = info.tree.trim(:,info.tree_path); % planned trims in the future Hp time steps
            info.trim_indices = info.trims_Hp(:,2);
