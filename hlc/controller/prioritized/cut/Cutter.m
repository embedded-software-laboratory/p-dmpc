classdef (Abstract) Cutter
    ...
    methods (Access = public)
        ...
        function directed_coupling_sequential = cut(obj, M, coupling_info, max_num_CLs)
            ...
            [belonging_vector, subgraphs_info] = obj.graph_partitioning_algorithm(M, coupling_info, max_num_CLs);

            % subgraphs is mergeable if the number of computation levels of the
            % new graph does not exceed the maximum allowed number.
            subgraphs_info = Cutter.graph_merging_algorithm(M, belonging_vector, subgraphs_info, max_num_CLs);
