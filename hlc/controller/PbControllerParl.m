classdef PbControllerParl < HLCInterface
    ...
    methods (Access = protected)
        function controller(obj)
            ...
            obj.scenario.timer.assign_priority = toc(assign_priority_timer);
            ...
            obj.scenario.num_couplings_between_grps = 0; % number of couplings between groups
            obj.scenario.num_couplings_between_grps_ignored = 0; % ignored number of couplings between groups by using lanelet crossing lanelets
            for iCoupling = 1:length([obj.scenario.coupling_info.veh_with_ROW])
                veh_ij = [obj.scenario.coupling_info(iCoupling).veh_with_ROW,obj.scenario.coupling_info(iCoupling).veh_without_ROW];
                is_same_grp = any(cellfun(@(c) all(ismember(veh_ij,c)),{obj.scenario.parl_groups_info.vertices}));
                if ~is_same_grp
                    obj.scenario.num_couplings_between_grps = obj.scenario.num_couplings_between_grps + 1;
                    if obj.scenario.coupling_info(iCoupling).is_ignored
                        obj.scenario.num_couplings_between_grps_ignored = obj.scenario.num_couplings_between_grps_ignored + 1;
                    end
                end
            end
