...
%   compiles both the priority-based Graph Saecrh and the centralized Graph
...
if centralized_compile
    if ~p.Results.centralized_recompile
        filepath_centralized = fullfile(filepath, 'build', [name '_centralized_mex.' mexext]);
...
if priority_compile
    if ~p.Results.priority_recompile    
        filepath_priority = fullfile(filepath, 'build', [name '_priority_mex.' mexext]);
...
if centralized_compile || priority_compile
    ...
    build_path = fullfile(filepath, 'build', 'cmake');
