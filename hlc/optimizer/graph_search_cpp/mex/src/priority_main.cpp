   ...
   private:
	...
	PriorityBasedResultStructMex run_priority_based(matlab::data::ObjectArray const& iter, matlab::data::CellArray const& vehicle_obstacles_cell_array, std::string const& Function) {
		...
		if (Function == "GraphSearchPBOptimal") {
			...
			if (!incremental_graph_search)
				graph_search = incremental_graph_search = new GraphSearchPBIncrementalOptimal<scenario_type>(_config, _mpa, std::move(vehicle_data), std::move(vehicle_obstacles_data));
			else {
	...
	PriorityBasedResultStructMex run_priority_based(matlab::data::ObjectArray const& iter, matlab::data::CellArray const& vehicle_obstacles_cell_array, std::string const& Function) {
		...
		switch (_config->scenario_type()) {
			...
			case SCENARIO_TYPE::Lanelet2: return run_priority_based<SCENARIO_TYPE::Lanelet2>(iter, vehicle_obstacles_cell_array, Function);
