...
namespace GraphBasedPlanning {
	class GraphSearchCentralizedOptimalPolymorphicSpeedHeuristic : private GraphSearchSpecialization {
		...
		void expand_node(std::uint8_t *const next_trims, std::uint8_t *const sizes, unsigned int *const multipliers, std::pmr::polymorphic_allocator<> &pa,
			...
			if (next_step < n_hp()) {
				for (unsigned int k = 0; k < n_children; ++k) {
					...
					init_new_node_other_heuristic(node, next_trims, next_step, new_node);
