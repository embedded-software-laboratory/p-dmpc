...
for i = 1:length(strategy_feasibility)
    ...
    options.strategy_enter_lanelet_crossing_area = strategy_feasibility{i}{2};
    options.amount = nVeh_max(i);
    ...
    % data processing
    e_feasibility_differentNumVehs{i} = EvaluationParl(options);
...
nVeh_max = cellfun(@(c) c.nVeh, e_feasibility_sameNumVehs);
