...
function results = spp_book(visu_options)
    ...
    opt.is_load_mpa = 0;
    opt.Hp = 3;
    opt.mpa_type = MpaType.single_speed;
    veh = Vehicle();
    model = BicycleModel(veh.Lf, veh.Lr);
    sce_rec_1 = Scenario;
    sce_rec_1_mpa = MotionPrimitiveAutomaton(model, opt);
    sce_rec_1.options = opt;

    export_fig_config = ExportFigConfig.spp_book_small( ...
        "paperheight", 3 ...
    );
    sce_rec_1_mpa.plot_mpa_over_time( ...
        sce_rec_1, ...
        'do_export', true, ...
    ...
    sce_rec_0 = Scenario;
    sce_rec_0_mpa = MotionPrimitiveAutomaton(model, opt);
    sce_rec_0.options = opt;
    sce_rec_0_mpa.plot_mpa_over_time( ...
        sce_rec_0, ...
        'do_export', true, ...
        'export_fig_cfg', export_fig_config ...
    );
