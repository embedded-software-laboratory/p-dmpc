...
% MOTIONPRIMITVEAUTOMATON   MotionPrimitiveAutomaton 
    ...
    methods
        function obj = MotionPrimitiveAutomaton(model, trim_set, offset_length, offset_width, dt, nveh, N, nTicks, recursive_feasibility)
            % Constructor
            ...
            for i = 1:n_trims
                for j = 1:n_trims
                    if obj.transition_matrix_single(i,j,1)
                        obj.maneuvers{i,j} = generate_maneuver(model, obj.trims(i), obj.trims(j), offset_length, offset_width, dt, nTicks);
                    end
                end
            end
