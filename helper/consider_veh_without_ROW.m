function scenario_v = consider_veh_without_ROW(scenario_v, iter, all_coupling_vehs_LP)
% CONSIDER_VEH_WITHOUT_ROW Stategies to let vehicle with the right-of-way
    ...
    for i_LP = 1:length(all_coupling_vehs_LP)
        ...
        switch scenario_v.strategy_consider_veh_without_ROW
            ...
            case '3'
                ...
                latest_msg_LP = scenario_v.rosSubs_trafficInfo{veh_LP,1}.LatestMessage;
                if latest_msg_LP.time_step > 0
                    % the message does not come from the initial time step
                    predicted_areas_LP = reshape(arrayfun(@(array) {[array.x';array.y']}, latest_msg_LP.predicted_areas),1,[]);
