function result = get_result_struct(options, scenario, controlled_vehicle_ids)
    ...
    result.vehicle_path_fullres = cell(options.amount, 0);
    result.trajectory_predictions = cell(options.amount, 0);
    result.belonging_vector = zeros(options.amount, 0); % belonging vector indicates to which group one vehicle belongs to
    result.priority_list = zeros(options.amount, 0);
