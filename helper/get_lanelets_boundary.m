...
% target_lanelet_boundaries: predicted lanelets boundaries
    ...
    if is_sim_lab
        ...
        left_bound = cellfun(@(c)c{1}(2:end,:)', target_lanelet_boundaries, 'UniformOutput',false);
        left_bound = [left_bound{:}];

        right_bound = cellfun(@(c)c{2}(2:end,:)', target_lanelet_boundaries, 'UniformOutput',false);
