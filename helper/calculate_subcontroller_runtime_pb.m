function runtimes = calculate_subcontroller_runtime_pb(result)
...
%                                       path in the coupling graph (longest means highest cumulative runtime)
    ...
    [nVeh, nSteps] = size(result.subcontroller_runtime);
    runtimes = zeros(1,nSteps);

    % build DAG from levels and coupling adjacency
    for i = 1 : nSteps
        runtime = result.subcontroller_runtime(:,i);
        % --- modified only for eval/prio-paper --
        groups = result.coupling{1}.groups;
        adjacency = result.coupling{1}.adjacency;
        visited = [];
