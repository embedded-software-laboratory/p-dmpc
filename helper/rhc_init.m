...
% RHC_INIT  Preprocessing step for RHC controller
        ...
        if ~initialized_reference_path
            for iVeh = 1:scenario.nVeh
                ...
                if scenario.options.isParl
                    ...
                    predicted_lanelets = get_predicted_lanelets(scenario.vehicles(iVeh),predicted_trims(1),x0,y0,scenario.mpa,scenario.dt, scenario.options.isParl, scenario.name);
    ...
    for iVeh=1:scenario.nVeh
        ...
        if (scenario.manual_vehicle_id == scenario.vehicle_ids(iVeh) && scenario.manual_mpa_initialized)
            [predicted_lanelets,reference,v_ref] = get_predicted_lanelets(scenario.vehicles(iVeh), trim_current, x0, y0, scenario.vehicles(iVeh).vehicle_mpa, scenario.dt, scenario.options.isParl, scenario.name);
...
%             iter.vRef(iVeh,:) = get_max_speed(scenario.vehicles(iVeh).vehicle_mpa,iter.trim_indices(iVeh));
            ...
            [predicted_lanelets,reference,v_ref] = get_predicted_lanelets(scenario.vehicles(iVeh), trim_current, x0, y0, scenario.mpa, scenario.dt, scenario.options.isParl, scenario.name);
