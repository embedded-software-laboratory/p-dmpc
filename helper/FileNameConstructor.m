classdef FileNameConstructor
    ...
    methods (Static)
        ...
        function mpa_instance_name = get_mpa_name(options)
            ...
            mpa_instance_name = ['MPA_', 'trims', char(options.mpa_type), '_Hp', num2str(options.Hp), '_T', num2str(options.dt_seconds)];
        ...
        function scenario_name = gen_scenario_name(options, i_vehicles)
            ...
            if isempty(options.result_name)
                ...
                if options.compute_in_parallel
                    scenario_name = ['veh_', num2str(options.path_ids(i_vehicles)), '_trims', char(options.mpa_type), '_Hp', num2str(options.Hp), '_dt', num2str(options.dt_seconds), '_nVeh', num2str(options.amount), '_T', num2str(options.T_end), '_', priority];
                else
                    scenario_name = ['trims', char(options.mpa_type), '_Hp', num2str(options.Hp), '_dt', num2str(options.dt_seconds), '_nVeh', num2str(options.amount), '_T', num2str(options.T_end), '_', priority];
                end
