classdef systemtests < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function test_plot_default(testCase, scenario_type)
            ...
            full_result = load(output_path, 'experiment_result').experiment_result;

            %verify without exporting
            plot_default(full_result, do_export = false);
            testCase.verifyTrue(true);

            %verify and check export too
            plot_default(full_result, do_export = true);
            testCase.verifyTrue(true);

            % verify export_video
            export_video(full_result);
            ...
            plot_partitioned_graph(full_result);
            testCase.verifyTrue(true);
