...
function search_tree = generate_tree(init_poses, target_poses, trim_indices, combined_graph, search_depth)
...
%     tf1 = (search_tree.depth() < search_depth);
    ...
    while ~tf2 ...
        ...
        parent = get_next_node_astar(search_tree, leaf_nodes);
        
        % Delete chosen entry from list of expandable nodes
        leaf_nodes(leaf_nodes == parent) = [];
        
        [leaf_nodes, search_tree, id, isgoals] = expand_tree(leaf_nodes, search_tree, parent, combined_graph, target_poses, visited_nodes, id, isgoals);
