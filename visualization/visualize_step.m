    ...
    if(parent > 1)
        ...
        for iVeh = 1 : n_veh
            ...
            if(~isempty(path))
                ...
                plot(parent_node(iVeh,tree.idx.x), parent_node(iVeh,tree.idx.y), 'o','Color', cur_color, 'MarkerSize',6,'MarkerFaceColor', cur_color);
                % text(parent_node(iVeh,tree.idx.x) - 0.1, parent_node(iVeh,tree.idx.y) - 0.1, num2str(parent), 'FontSize', 6);
            end
        end  
    else
        for iVeh = 1 : n_veh
